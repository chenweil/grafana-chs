...
func (hs *HTTPServer) GetDashboard(c *m.ReqContext) Response {
	...
	if err != nil {
		return Error(500, "检查仪表板是否由用户标记时出错", err)
	}
	...
	if err != nil {
		return Error(500, "检查仪表板是否已设置时出错", err)
	}
	...
	if provisioningData != nil {
		...
		if err != nil {
			...
			hs.log.Warn("无法创建ProvisionedExternalId", "err", err)
		}
	}
...
func getDashboardHelper(orgID int64, slug string, id int64, uid string) (*m.Dashboard, Response) {
	...
	if err := bus.Dispatch(&query); err != nil {
		return nil, Error(404, "未找到仪表板", err)
	}
...
func (hs *HTTPServer) PostDashboard(c *m.ReqContext, cmd m.SaveDashboardCommand) Response {
	...
	if err != nil {
		...
		if pluginErr, ok := err.(m.UpdatePluginDashboardError); ok {
			message := "仪表板属于插件 " + pluginErr.PluginId + "."
			// look up plugin name
			if pluginDef, exist := plugins.Plugins[pluginErr.PluginId]; exist {
				message = "仪表板属于插件 " + pluginDef.Name + "."
			}
...
func GetHomeDashboard(c *m.ReqContext) Response {
	...
	if err != nil {
		return Error(500, "无法加载主页仪表板", err)
	}
	defer file.Close()
...
func GetDashboardVersions(c *m.ReqContext) Response {
	...
	if err := bus.Dispatch(&query); err != nil {
		return Error(404, fmt.Sprintf("找不到dashboardId的版本 %d", dashID), err)
	}
	...
	for _, version := range query.Result {
		...
		if version.RestoredFrom > 0 {
			version.Message = fmt.Sprintf("从版本恢复 %d", version.RestoredFrom)
			continue
		}
...
func GetDashboardVersion(c *m.ReqContext) Response {
	...
	if err := bus.Dispatch(&query); err != nil {
		return Error(500, fmt.Sprintf("未从dashboardId %d 的仪表版本为 %d 信息",  dashID, query.Version), err)
	}
...
func (hs *HTTPServer) RestoreDashboardVersion(c *m.ReqContext, apiCmd dtos.RestoreDashboardVersionCommand) Response {
	...
	saveCmd.Message = fmt.Sprintf("从 %d 版本恢复", version.Version)
