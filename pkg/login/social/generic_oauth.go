...
func (s *SocialGenericOAuth) FetchPrivateEmail(client *http.Client) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("获取电子邮件地址出错: %s", err)
	}
...
func (s *SocialGenericOAuth) FetchTeamMemberships(client *http.Client) ([]int, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("获得团队成员资格时出错: %s", err)
	}
...
func (s *SocialGenericOAuth) FetchOrganizations(client *http.Client) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("获得团队时出错: %s", err)
	}
...
func (s *SocialGenericOAuth) UserInfo(client *http.Client, token *oauth2.Token) (*BasicUserInfo, error) {
	...
	if !s.extractToken(&data, token) {
		...
		err = json.Unmarshal(response.Body, &data)
		if err != nil {
			return nil, fmt.Errorf("解码用户信息JSON时出错: %s", err)
		}
	}
	...
	if !s.IsTeamMember(client) {
		return nil, errors.New("用户不是所需团队之一的成员")
	}

	if !s.IsOrganizationMember(client) {
		return nil, errors.New("用户不是所需组织之一的成员")
	}
...
func (s *SocialGenericOAuth) extractToken(data *UserInfoJson, token *oauth2.Token) bool {
	...
	if idToken == nil {
		s.log.Debug("找不到id_token", "token", token)
		return false
	}
	...
	if matched == nil {
		s.log.Debug("id_token不是JWT格式", "id_token", idToken.(string))
		return false
	}

	payload, err := base64.RawURLEncoding.DecodeString(matched[2])
	if err != nil {
		s.log.Error("错误base64解码id_token", "raw_payload", matched[2], "err", err)
		return false
	}

	err = json.Unmarshal(payload, data)
	if err != nil {
		s.log.Error("解码id_token JSON时出错", "payload", string(payload), "err", err)
		return false
	}

	email := s.extractEmail(data)
	if email == "" {
		s.log.Debug("在id_token中找不到电子邮件", "json", string(payload), "data", data)
		return false
	}

	s.log.Debug("收到id_token", "json", string(payload), "data", data)
	return true
}
