...
func NewDashboardProvisionerImpl(configDirectory string) (*DashboardProvisionerImpl, error) {
	...
	if err != nil {
		return nil, errutil.Wrap("无法读取仪表板配置", err)
	}

	fileReaders, err := getFileReaders(configs, logger)

	if err != nil {
		return nil, errutil.Wrap("无法初始化文件读取器", err)
	}
	...
	for _, reader := range provider.fileReaders {
		if err := reader.startWalkingDisk(); err != nil {
			if os.IsNotExist(err) {
				...
				provider.log.Warn("未能提供配置", "name", reader.Cfg.Name, "error", err)
				return nil
			}

			return errutil.Wrapf(err, "未能提供配置 %v", reader.Cfg.Name)
		}
	}
...
func getFileReaders(configs []*DashboardsAsConfig, logger log.Logger) ([]*fileReader, error) {
	...
	for _, config := range configs {
		...
		case "file":
			...
			if err != nil {
				return nil, errutil.Wrapf(err, "无法创建配置文件阅读器 %v", config.Name)
			}
			readers = append(readers, fileReader)
		default:
			return nil, fmt.Errorf("类型 %s 不受支持", config.Type)
		}
	}
